...
# Load the dataset
df = pd.read_csv('../../CSVs/fantasy_points/ODI_MatchWise_fantasy_points.csv', low_memory=False)
...
for index, row in df.iterrows():
    ...
    df.at[index, 'career_bowler_average_odi'] = (stats['total_runs_conceded'] / (stats['total_balls_bowled'] / 6)) if stats['total_balls_bowled'] > 0 else 0
    df.at[index, 'career_bowler_economy_rate_odi'] = (stats['total_runs_conceded'] / stats['total_wickets']) if stats['total_wickets'] > 0 else 0
...
df.to_csv('../../CSVs/Final/ODI/career_odi.csv', index=False)
