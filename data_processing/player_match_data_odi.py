...
def process_player_match_data(designation_file, odi_matchwise_file, output_file):
    """
    Merges designation data with ODI matchwise fantasy points data to create a player match data file which includes match_id, player_id, player_name, fantasy_points, team_name, role, date, venue.
    
    Parameters:
        designation_file (str): Path to the designation CSV file.
        odi_matchwise_file (str): Path to the ODI matchwise fantasy points CSV file.
        output_file (str): Path to save the processed CSV file.
    """
    try:
        # Load the necessary datasets
        designation_df = pd.read_csv(designation_file)
        odi_matchwise_df = pd.read_csv(odi_matchwise_file, low_memory=False)
        ...
        if not required_columns.issubset(odi_matchwise_df.columns):
            raise ValueError("'player_id' or 'player_name' not found in ODI MatchWise data.")
...
#     '../data/interim/ODI_MatchWise_fantasy_points.csv',
#     '../data/processed/ODI/player_match_data_odi.csv'
# )
